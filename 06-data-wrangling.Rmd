...
### dplyr 패키지 소개
+ 데이터 가공, 탐색적 자료분석 등에 매우 유용하고 편리한 함수 제공하는 패키지
+ 적용 대상에 따른 dplyr 함수
  + **행**: filter(), arrange()
...
```{r eval=FALSE}
flights %>% mutate(prev_dep = lag(dep_delay))                         # 이전 비행기의 출발 지연 시간
flights %>% mutate(next_dep = lead(dep_delay))                        # 다음 비행기의 출발 지연 시간
...
flights %>% mutate(flight_length = if_else(air_time >= 180, "Long Flight", "Short Flight"))
